...
namespace XFGloss.Droid.Renderers
{
	...
	internal class DroidXFGlossCellRenderer : XFGlossCellRenderer<AView>, IGradientRenderer
	{
		...
		protected override void UpdateProperties(Cell cell, AView nativeCell, string propertyName)
		{
			...
			if (propertyName == null || propertyName == CellGloss.BackgroundColorProperty.PropertyName)
			{
                ...
                // mwj
                Color rippleColor = (Color)cell.GetValue(CellGloss.RippleColorProperty);
                // mwj - eom
                ...
                if (propertyName != null || bkgrndColor != Color.Default || rippleColor != Color.Default)
                //mwj eom
                {
                    ...
                    //mwj
                    //AColor aRippleColor = 
                    //Android.Util.TypedValue x = new Android.Util.TypedValue();
                    //var theme = new Resources.Theme();
                    //Resources.Theme.ResolveAttribute(Android.Resource.Attribute.ColorControlHighlight, x, true);
                    ...
                    if (Android.OS.Build.VERSION.SdkInt >= Android.OS.BuildVersionCodes.Lollipop)
					{
                        ...
                        //var ripple = rippleColor != Color.Default ? BackgroundRippleDrawable.Create(aBkColor, rippleColor.ToAndroid()) : BackgroundRippleDrawable.Create(aBkColor);
                        var ripple = BackgroundRippleDrawable.Create(aBkColor, rippleColor.ToAndroid());
                        //mwj eom
                        nativeCell.SetOnTouchListener(ripple);
						nativeCell.Background = ripple;
					}
		...
		{            
            ...
            var alpha = (Android.OS.Build.VERSION.SdkInt >= Android.OS.BuildVersionCodes.Lollipop) ? 64 : 32;            

            if (backgroundColor == AColor.Transparent)
            {
			...
			//pink
			//return new AColor(230, 34, 213, alpha);

			// green
			//return new AColor(34, 230, 164, alpha);

			//return new AColor(Math.Max(backgroundColor.R - 10, 0), Math.Max(backgroundColor.G - 10, 0), Math.Max(backgroundColor.B - 10, 0), alpha);
			return new AColor(backgroundColor.R, backgroundColor.G, backgroundColor.B, alpha);
		...
		[Android.Runtime.Preserve(AllMembers = true)]
		public class BackgroundRippleDrawable : RippleDrawable, AView.IOnTouchListener
		{
            ...
            // mwj - ctor to specify ripple color also
            public static BackgroundRippleDrawable Create(AColor backgroundColor, AColor rippleColor)
            {
                return BackgroundRippleDrawable.Create(new ColorDrawable(backgroundColor), rippleColor);
            }
            // mwj eom
	...
	public class XFGlossViewCellRenderer : ViewCellRenderer
	{
		...
		protected override AView GetCellCore(Cell item, AView convertView, AViewGroup parent, Context context)
		{
            ...
            // if bg color is set, but ripple color is not, we want to use the default ripple color
            //if((Color)item.GetValue(CellGloss.BackgroundColorProperty) != Color.Default && (Color)item.GetValue(CellGloss.RippleColorProperty) == Color.Default)
            //{
            //    Android.Util.TypedValue x = new Android.Util.TypedValue();
            //    context.Theme.ResolveAttribute(Android.Resource.Attribute.ColorControlHighlight, x, true);

            //    Android.Util.TypedValue y = new Android.Util.TypedValue();
            //    context.Theme.ResolveAttribute(XFGloss.Droid.Resource.Attribute.colorControlHighlight, y, true);                
            //}
